                            ...
                            <table id="tabla" class="table table-bordered table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="col-1" data-priority="2">Estado</th>
                                ...
                                <tbody>
                                    @forelse($tickets as $key => $ticket)
                                        <tr>
                                            ...
                                            <td class="h5">
                                                @if ($ticket->status==0)
                                                    <span class="px-2 badge bg-secondary rounded-pill align-text-top">Pendiente</span>
                                                @elseif ($ticket->status==1)
                                                    <span class="px-2 text-light badge bg-warning rounded-pill align-text-top">Asignado</span>
                                                    @else
                                                    <span class="px-2 badge bg-success rounded-pill align-text-top">Completado</span>
                                                @endif
                                            </td>
                                            <td>
                                                <div>
                                                    @if ($ticket->status==0)
                                                        <a href="{{route('ticket.show', $ticket->id)}}" type="button" class="text- d-flex justify-content-center align-items-center btn btn-sm btn-outline-secondary editable">
                                                    @else 
